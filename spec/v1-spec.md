...
- Pipeline mode: `;;` separator to queue multiple commands, execute atomically (changed from `|` to avoid conflicts with Redis key names containing pipe characters)
...
| `max_keys_in_memory` | `500000` | Maximum keys held in the namespace tree before capping |
...
| Memory footprint | < 50MB RSS at idle. `NamespaceTree` enforces a `max_keys_in_memory` cap (default 500,000). If the scan exceeds the cap, a "Too many keys to display fully — use filter" banner is shown and further key insertions are dropped. |
...
- **Error propagation:** All Redis actions return `Result`. Errors are pushed into the `StatusBar`'s notification queue and rendered as a transient toast at the bottom of the screen (fades after 3s). Every action handler in `App` maps `Err` → `status_bar.push_error(msg)` so no Redis failure is silently swallowed.
...
| Shell parsing | `shlex` | Shell-like tokenization for REPL piped commands |
...
**PubSub** — dedicated Redis connection (subscribe-only, per Redis protocol). Subscription events streamed through a separate `mpsc::UnboundedSender<PubSubMessage>` (preserves ordering; single consumer). If the consumer falls behind, oldest messages are dropped to bound memory.

**All channels and buffers have explicit capacity limits:**
- Scan batch channel (`mpsc`): capacity 64 batches; sender drops oldest if receiver is full (non-blocking `try_send`)
- PubSub message deque: max 5,000 messages; oldest dropped on overflow
- REPL output deque: max 500 `ReplEntry` items; oldest dropped
- REPL history: max 1,000 entries
- Slow log entries: max 10,000 entries (fetched from server, display-bounded by viewport)
- Sparkline buffers: 60 samples (circular)
...
pub struct App {
    ...
    pub config: Config,
    pub active_tab: Tab,
    pub overlay: Option<AppMode>, // at most one modal overlay active at a time
...
- When `self.overlay` is `Some(...)`, render the overlay on top of the base layout. `Esc` pops the overlay (unless the overlay handles it, e.g. Confirm dialog). Only one overlay is active at a time — opening a new overlay replaces the current one.
...
// Assert: max_keys_in_memory() == 500000
...
**Additional test coverage for Milestone 0:**
- **`test_concurrent_mutation`** *(integration)*: Verify that external key mutations (via separate client) are handled gracefully during value inspection
- **`test_resize_extreme_sizes`** *(unit)*: Test rendering at extreme terminal sizes (40x10, 300x200) for all components
- **`test_cli_flag_conflicts`** *(unit)*: Verify that conflicting CLI flags (e.g., `--url` + `--profile`) return an error
...
impl PasswordRef {
    /// Called once at connect time. The resolved password is stored
    /// in the RedisClientHandle and never re-resolved. This avoids
    /// repeated keychain prompts and is thread-safe.
    pub fn resolve(&self) -> Result<String>
    /// Note: Password resolution happens once during `RedisClient::connect()`.
    /// The resolved password is cached in the connection handle to avoid
    /// repeated keychain prompts and ensure thread-safety.
}
...
- **Cluster note:** SCAN on a cluster connection may return duplicate keys if the keyspace is resharded mid-scan (redis-rs transparently routes SCAN to each node; cursor state can overlap). The `NamespaceTree` deduplicates on `full_name` via `BTreeMap`. This is safe for v1 — duplicates are silently ignored.
...
impl NamespaceTree {
    ...
    pub fn insert(&mut self, key: KeyEntry) -> bool  // returns false if at capacity
...
- `fn start_scan(client: RedisClient, config: &Config) -> (mpsc::Receiver<Vec<KeyEntry>>, CancellationToken)`
...
- Accepts a `CancellationToken` that is checked before each SCAN cursor iteration; cancellation stops the loop immediately and drops the sender. The returned token is stored in `App` and triggered on connection switch or app shutdown.
...
pub struct StatusBar {
    ...
    pub error_notifications: VecDeque<(String, Instant)>, // transient toasts, 3s TTL
}

impl StatusBar {
    pub fn push_error(&mut self, msg: String)
    pub fn drain_expired_toasts(&mut self) // removes notifications older than 3s
    // Error propagation: the `error_notifications` queue is the single point where all Redis action errors surface.
    // Every action handler in `App` maps `Err` → `status_bar.push_error(msg)`.
    // During rendering, shows most recent non-expired error as transient toast (red text, auto-fades).
}
...
Pipeline parsing:
- Split input on ` ;; ` (space-semicolon-semicolor-space)
- Each segment tokenized via `shlex::split()` (respects quoted strings: `"foo bar"` = one token, handles nested quotes correctly)
...
// Note: Uses `;;` separator instead of `|` to avoid conflicts with pipe characters in key names
```
